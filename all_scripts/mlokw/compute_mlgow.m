...
for i = 2:jdm-1
 for j = 2:idm-1
mlg(i,j) = (ml(i,j+1) - ml(i,j-1))/pscx(i,j) + (ml(i+1,j) - ml(i-1,j))/pscx(i,j);
 end
end
...
f1 = figure
scatter(M,Q)
title(strcat('OW and ML Gradient (cell size: ',num2str(gs),'). Day: ', num2str(lday)))
file = strcat('./plot/scatter_mlgow',gs,'_016_archv.0008_',lday,'_00.a');
...
f2 = figure
imagesc(MQ)
title(strcat('OW (cell size: ',num2str(gs),'). Day: ', num2str(lday)))
...
f3 = figure
imagesc(MM)
title(strcat('ML Gradient (cell size: ',num2str(gs),'). Day: ', num2str(lday)))
file = strcat('./plot/mlg_grid_',gs,'_016_archv.0008_',lday,'_00.a');
label = strcat(file,'.eps');
