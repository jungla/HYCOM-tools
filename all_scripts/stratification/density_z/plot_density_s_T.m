...
% axes( 'Position',[.005 .005 .99 .99],'xtick',[],'ytick',[],'box','on','handlevisibility','off')
 s0 = subplot(2,1,1,'Position',[0.2 0.5 0.6 0.3]),
...
if (arch == 1)
 label = strcat('../../stratification/density_z/plot/',num2str(dmax),'/high-res/vort_dens_',num2str(dmax),'_',lyear,'_',lday,'_h_',R,num2str(s),'s.eps')
% title(strcat(['Density section and Vorticity field of region ',R,' high-res. Year ',lyear,', day ',lday]));
else
 label = strcat('../../stratification/density_z/plot/',num2str(dmax),'/low-res/vort_dens_',num2str(dmax),'_',lyear,'_',lday,'_l_',R,num2str(s),'s.eps')
% title(strcat(['Density section and Vorticity field of region ',R,' low-res. Year ',lyear,', day ',lday]));
end

 axis xy;

 s1 = subplot(2,1,2,'Position',[0.2 0.1 0.6 0.3]),
 [p1,p1] = contourf(XI,YI,ZI);
