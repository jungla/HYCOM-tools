program energy_s
 ...
 REAL*4,  ALLOCATABLE :: RSCX(:,:),RSCY(:,:),plon(:,:),plat(:,:),lon(:),lat(:)
 REAL*4,  ALLOCATABLE :: Q(:,:),H(:,:),ub(:,:),vb(:,:),u(:,:),v(:,:),ut(:,:),vt(:,:),ubt(:,:),vbt(:,:)
 ...
 REAL*4       Rt,Volume,TV,TH,TE,energy
 REAL*4       Vl(nl),Hl(nl)
 ...
 case (0)    ! vertical section - average in x and y, 1D vector output
  ...
  do k=1,nl
   ...
   do i=x1,x2
    ...
    ii = i-x1+1
    jj = j-y1+1
    if(H(i,j) < 10**5 .and. abs(ub(ii,jj)) < 10**4 .and. abs(ut(i,j)) < 10**4 .and. &
        abs(vt(i,j)) < 10**4 .and. abs(vb(ii,jj)) < 10**4) then
     energy = 0.5*((ub(ii,jj) + ut(i,j))**2 + (vb(ii,jj) + vt(i,j))**2)
     Volume   = H(i,j) * rscx(i,j) * rscy(i,j)  ! volume single cube
     TE       = TE + energy * Volume            ! mass   total  
   ...
   enddo
    ...
    if (n > 0) then
      Vl(k) = TE/TV                             ! energy per unit volume
      if (k > 1) then
       Hl(k) = TH/n + Hl(k-1)
      else 
       Hl(k) = TH/n
...
!  9010 format(I2,2X,D10.5,2X,F10.5)
 enddo
 close(21)
