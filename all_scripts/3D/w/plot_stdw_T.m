...
%maxw = quantile(w(~isnan(w)),0.999)
%minw = quantile(w(~isnan(w)),0.001)
...
if (arch == 1)
 label = strcat('./plot/high-res/w_std_',depth,'_',lyear,'_',lday,'_h_',R,'.eps')
else
 label = strcat('./plot/low-res/w_std_',depth,'_',lyear,'_',lday,'_l_',R,'.eps')
end 
