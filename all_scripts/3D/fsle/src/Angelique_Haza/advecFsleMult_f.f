...
c --- model-output loop
       do l=l0,min(l0+lmt-skip,151),skip
       print*,'-------------------------------------------------------'


c --- read micom fields and change axes
      if (l.le.dayTr1) then
       call read_uv(l,u1,v1)
      else if (l.le.dayTr2) then
       call read2_uv(l,u1,v1)
      else  if (l.le.dayTr3) then
       call read3_uv(l,u1,v1)
      else if (l.le.dayTr4) then
       call read4_uv(l,u1,v1)
      else
       call read5_uv(l,u1,v1)
      endif
       print*,'..u1 read'

c      lup=l+1
c      lup=min(l+skip,day0-1+lmt)
      lup=l+skip
      if (lup.le.dayTr1) then
       call read_uv(lup,u2,v2)
      else if (lup.le.dayTr2) then
       call read2_uv(lup,u2,v2)
      else if (lup.le.dayTr3) then
       call read3_uv(lup,u2,v2)
      else if (lup.le.dayTr4) then
       call read4_uv(lup,u2,v2)
      else
       call read5_uv(lup,u2,v2)
      endif
      print*,'..u2 read'
...
c --- particles ipp loop 
     ...
     .     yin=.true.
         if(.not.(xin.and.yin)) then
          ...
          ii=int(x0(ipp))
          jj=int(y0(ipp)) 
c          CALL EULER(n,m,X0(ipp),Y0(ipp),u1,v1,u2,v2,IT,ITI,DT,dx,dy)
