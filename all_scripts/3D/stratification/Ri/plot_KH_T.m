...
tvort  = binaryread(file2,idm,jdm,ijdm,1);
Rot    = tvort./f;

Ro = Rot(Y1:Y2,X1:X2);
Ri = Rit(Y1:Y2,X1:X2);

 minRo   = quantile(Ro(~isnan(Ro)),.001);
 maxRo   = quantile(Ro(~isnan(Ro)),.999);
...
%%%%%%%%%%%%%%%%%%%%%% Ri < 0
 ...
 [r2,r2] = contour(lon,lat,Ri,[0.25 0.25]);
 set(r2,'Color',[0 0 0],'LineStyle','-','ShowText','off');
...
if (arch == 1)
 label = strcat('./plot/high-res/KH_',depth,'_',lyear,'_',lday,'_h_',R,'.eps')
else
 label = strcat('./plot/low-res/KH_',depth,'_',lyear,'_',lday,'_l_',R,'.eps')
end 
