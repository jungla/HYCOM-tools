...
subroutine tridiag(jmx,a,b,c,f,g)
...
subroutine rhs(O,uscx,vscy,pscx,pscy,Bf,G)
! O enter as vertical velocity and exits as RHS
...
do k=1,nl
 do i=2,idm-1
  ...
  E(i,j,k) = A*O(i+1,j,k)+B*O(i-1,j,k)+C*O(i,j+1,k)+D*O(i,j-1,k)-G(i,j,k)
  endif
  enddo
 enddo
enddo
