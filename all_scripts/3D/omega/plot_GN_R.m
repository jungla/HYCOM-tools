...
if (arch == 1)
 file   = strcat('/media/sdd1/hycom/scripts/3D/omega/output/high-res/GN_h_016_archv.',lyear,'_',lday,'_00.a');
 file1  = strcat('/media/sdd1/hycom/GSa0.02_3D/016_archv.',lyear,'_',lday,'_00_3zr.A');
else
 file   = strcat('/media/sdd1/hycom/scripts/3D/omega/output/low-res/GN_l_016_archv.',lyear,'_',lday,'_00.a');
 file1  = strcat('/media/sdd1/hycom/GSa0.08_3D/archv.',lyear,'_',lday,'_00_3zr.A');
end

GNt  = binaryread(file,idm,jdm,ijdm,depthid);
...
% orient landscape;
 ...
 maxGN   = quantile(GN(~isnan(GN)),.99);
 minGN   = quantile(GN(~isnan(GN)),.01);


%%%%%%%%%%%%%%%%%%%%%% GN
% [p1,p1] = contourf(lon,lat,GN,50);
 p1 = imagesc(lon,lat,GN);
 axis xy;
...
% plot GN values for instabilities
 [r0,r0] = contour(lon,lat,GN,[1 1]);
 set(r0,'Color',[0 0 0],'LineStyle','-','ShowText','off');

 [r1,r1] = contour(lon,lat,GN,[2 2]);
...
if (arch == 1)
 label = strcat('/media/sdd1/hycom/scripts/3D/omega/plot/high-res/GN_',depth,'_',lyear,'_',lday,'_h_',R,'.eps')
else
 label = strcat('/media/sdd1/hycom/scripts/3D/omega/plot/low-res/GN_',depth,'_',lyear,'_',lday,'_l_',R,'.eps')
end 
