...
integer,parameter       :: ni=1,nf=10                                                  ! # layers, # scans, projection factor
integer                 :: nt,ns,kl
...
pscx = pscxt(X1:X2,Y1:Y2)
pscy = pscyt(X1:X2,Y1:Y2)
qscx = qscxt(X1:X2,Y1:Y2)
qscy = qscyt(X1:X2,Y1:Y2)
...
do k = 1,nt
 ...
 Bf(:,:,k) = N2(:,:,k)/(f(:,:)**2)
 write(*,*) depth(k), N2(10,10,k), G(10,10,k),Bf(10,10,k)
...
!write(*,*) s,O(10,10,nt),Bf(10,10,nt),G(10,10,nt)
