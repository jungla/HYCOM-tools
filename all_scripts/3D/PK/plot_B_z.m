...
fr = -binaryread(filer,ids,jds,ijds,did)*9.81./1000;

%%%%%%%%%%%%%%%%%%%%%% filter B

[bx,by] = gradient(smooth2(fr,2));

%B(did) = avg_region(sqrt((bx./pscx).^2+(by./pscy).^2),pscx,pscy,1,ids,1,jds,0);
B(did) = avg_region(sqrt((bx./pscx).^2+(by./pscy).^2),pscx,pscy,1,ids,1,jds,0);
dbdxa(did) = avg_region(sqrt((bx./pscx).^2),pscx,pscy,1,ids,1,jds,0);
dbdya(did) = avg_region(sqrt((by./pscy).^2),pscx,pscy,1,ids,1,jds,0);
dbdx(did) = avg_region((bx./pscx),pscx,pscy,1,ids,1,jds,0);
dbdy(did) = avg_region((by./pscy),pscx,pscy,1,ids,1,jds,0);
...
% ylabel('<|\nabla \bar{b}|>_{xy}','FontSize',21);
 ylabel('<|grad(B)|>_{xy}','FontSize',21);
 set(gca,'FontSize',21)
...
% ylim([-1*10^-8 5*10^-8]);
 ...
 label = strcat('./plot/B_z_',num2str(dmax),'_',R,'.eps')
