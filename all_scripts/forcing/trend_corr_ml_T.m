...
%  filer = strcat('../../GSa0.08_3Da/archv.',lyear,'_',lday,'_00_3zr.A');
 ...
 mld   = hycomread(file,idm,jdm,ijdm,6); 
...
% land = landt(Y1:Y2,X1:X2);
 ...
 mld = reshape(mld,ids*jds,1);
% C  = reshape(C,ids*jds,1);
 u  = reshape(u,ids*jds,1);
 fs = reshape(fs,ids*jds,1);
 ft = reshape(ft,ids*jds,1);

r = corrcoef(mld,u,'rows','pairwise')
 fuC(itime) = r(1,2);
r = corrcoef(mld,fs,'rows','pairwise')
 fsC(itime) = r(1,2);
r = corrcoef(mld,ft,'rows','pairwise')
 ftC(itime) = r(1,2);
...
fileostu = strcat('./plot/trend_corr_ml_',G,'.eps');
